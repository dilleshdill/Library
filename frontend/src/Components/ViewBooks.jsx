  ...
  const fetchBooksAndFavorites = async () => {
    try {
      ...
      const searchItem = await axios.get(
        `${import.meta.env.VITE_DOMAIN}/getSearch?email=${email}`
      );
      ...
      if (libraryId) {
        ...
        booksResponse = await axios.get(
          `${import.meta.env.VITE_DOMAIN}/admin/book?bookName=${searchTerm}`
        );
      }
      ...
      if (selectedFilters.length > 0) {
        const lowerCaseCategories = selectedFilters.map((cat) =>
          cat.toLowerCase()
        );
        filteredBooks = filteredBooks.filter(
          (book) =>
            book.catogory &&
            lowerCaseCategories.includes(book.catogory.toLowerCase())
        );
      }
      ...
      if (email) {
        ...
        const wishlistResponse = await axios.get(
          `${import.meta.env.VITE_DOMAIN}/books/wishlist?email=${email}`
        );
  ...
  const toAddFavorite = async (book, event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setCartBooks((prev) =>
          prev.filter((item) => item.book_name !== book.book_name)
        );
      }
    } catch (error) {
  ...
  const toAddWishList = async (book, event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_DOMAIN}/books/wishlist`,
        {
          email,
          book_name: book.book_name,
          book_url: book.book_url,
          ...
          <>
            ...
            <div className="flex flex-wrap justify-start m-auto md:pl-[2%] ">
              {books.map((book) => (
                <div
                  key={book._id}
                  className="w-full md:w-[30%] lg:w-[18%] mb-6 mr-6"
                >
                    ...
                    <div className="mt-4 px-5 pb-5">
                      ...
                      <div className="flex items-center justify-between pt-3">
                        {cartBooks.find((item) => item.bookId === book._id) ? (
                          ...
                          <button
                            className="!border-gray-500 !text-gray-400 !text-[15px] bg-gray-200 cursor-not-allowed !pt-1 !pb-1 !mt-2"
                            disabled
                          >
                            Out of stock
                          </button>
