...
const AddressForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post("http://localhost:5002/address", {
                email, name, phone_no, street, district, state, pincode, village
            });
    ...
    const handleDelete = async (_id) => {
        try {
            ...
            const response = await axios.post("http://localhost:5002/address/remove",{email,_id})
            if (response.status === 200) {
                setAddress(prevAddresses => prevAddresses.filter(item => item._id !== _id));
        ...
        <>
            ...
            <div className="flex flex-col w-screen md:flex-row items-start md:items-baseline justify-center md:justify-between min-h-screen p-6 bg-gray-100 dark:bg-gray-900">
                ...
                <div className="w-screen md:w-[70%]">
                    <h2 className="text-xl font-semibold mb-2 text-gray-800 dark:text-white">Select Address</h2>
                    {address.length === 0 ? (
                        <p className="text-gray-600 dark:text-gray-300">No address found. Please add a new address.</p>
                    ) : (
                        address.map((eachItem) => (
                            <div key={eachItem._id} className="flex justify-between items-center space-x-3 p-3  shadow-sm rounded-md bg-white dark:bg-gray-800 mb-2">
                                <div className="flex items-center">
                                    ...
                                    />
                                    <div className="ml-3">
                                        <p className="text-gray-800 dark:text-white font-medium">{eachItem.name}</p>
                                        <p className="text-gray-600 dark:text-gray-300">{eachItem.street}, {eachItem.district}, {eachItem.state} - {eachItem.pincode}</p>
                                    </div>
                                </div>
                                <Popup
                                    trigger={
                                        <button 
                                        className="text-red-500 !bg-transparent" style={{ border: 0, outline: 0 }}
                                        >
                                            Delete
                                        </button>
                                    } 
                                        ...
                                        <div className="p-6 pt-0 text-center">
                                            ...
                                            <div className="flex justify-center">
                                                <button
                                                    ...
                                                    className="text-white !bg-red-500 hover:bg-red-700 font-medium rounded-lg text-base px-4 py-2.5 mr-2" style={{ border: 0, outline: 0 }}
                                                >
                                                    Yes, Delete
                                                </button>
                                                <button
                                                    onClick={close}
                                                    className="text-gray-900 !bg-gray-300   font-medium rounded-lg text-base px-3 py-2.5" style={{ border: 0, outline: 0 }}
                                                >
                                                    Cancel
                                                </button>
                                            </div>
                    ...
                    {isOpen && (
                        <div className="fixed inset-0 flex items-center justify-center bg-gray-100  bg-opacity-10">
                            <div className="relative bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-lg">
                                <button onClick={() => setIsOpen(false)} className="absolute top-3 right-3 text-gray-600 dark:text-gray-300 text-xl">&times;</button>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                        ...
                                        <div>
                                            ...
                                            <input type="tel" placeholder="Enter phone number" value={phone_no} onChange={(e) => setPhoneNo(e.target.value)}
                                                className="w-full p-3 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white"/>
                                        </div>
                                    </div>
                                    <div>
                ...
                <CustomerDetails selectedAddress={address.find(addr => addr._id === selectedAddress)} />
            </div>
        </>
    );
};
